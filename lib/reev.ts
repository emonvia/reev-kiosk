...
try {
  const parsed = new URL(REEV_API_URL);
  if (
    process.env.NODE_ENV === "production" &&
    parsed.protocol !== "https:"
  ) {
    throw new Error(
      `REEV_API_URL must use HTTPS in production (got ${parsed.protocol})`,
    );
  }
} catch (e) {
  if (e instanceof TypeError) {
    throw new Error(`REEV_API_URL is not a valid URL: "${REEV_API_URL}"`);
  }
  throw e;
}

// Auth endpoint derived from the API base URL
const REEV_AUTH_URL = REEV_API_URL.replace(/\/+$/, "") + "/auth";
...
async function getAccessToken(): Promise<string> {
  ...
  const res = await fetch(REEV_AUTH_URL!, {
    ...
    body: JSON.stringify({
      clientId: REEV_CLIENT_ID!,
      clientSecret: REEV_CLIENT_SECRET!,
    }),
...
async function reevFetch<T>(
  ...
  // (e.g., "http://host/api/mock" + "partner/x" → "http://host/api/mock/x")
...
export async function getCustomers(page = 1, pageSize = 100) {
  return reevFetch<PageResponse<Customer>>("/customers", {
    page: clampPage(page),
...
export async function getChargingCards(
  ...
  return reevFetch<PageResponse<ChargingCard>>("/charging-cards", {
    page: clampPage(params.page),
    pageSize: clampPageSize(params.pageSize, 50),
    customerId: params.customerId,
  });
