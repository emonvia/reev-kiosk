...
describe("reev API client", () => {
  ...
  beforeEach(() => {
    ...
    process.env = {
      ...
      REEV_AUTH_URL:
        "https://auth.test.reev.com/realms/partner-api/protocol/openid-connect/token",
  ...
  describe("environment validation", () => {
    ...
    it("throws if REEV_AUTH_URL is missing", async () => {
      delete process.env.REEV_AUTH_URL;
      await expect(() => import("../reev")).rejects.toThrow(
        "Missing required REEV_AUTH_URL",
      );
    });
  ...
  describe("getChargingStations", () => {
    it("calls the correct endpoint with default params", async () => {
      ...
      const apiCall = fetchSpy.mock.calls.find(
        (c: [string]) => !c[0].includes("/openid-connect/token"),
      )!;
      const calledUrl = new URL(apiCall[0]);
      expect(calledUrl.pathname).toBe("/partner/charging-stations");
  ...
  describe("error handling", () => {
    ...
    it("sends Authorization header with Bearer token from OAuth2", async () => {
      ...
      const tokenCall = fetchSpy.mock.calls.find((c: [string]) =>
        c[0].includes("/openid-connect/token"),
      )!;
